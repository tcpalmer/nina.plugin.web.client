...
class AutofocusChart extends React.Component {
  ...
  getTrendLines(autofocus, points, calculatedPoint, domain) {
    ...
    const leftTrendLineEq = this.getTrendLineEquation(autofocus.Fittings.LeftTrend);
    const leftTrendLine = this.generateTrendLinePoints(true, points, calculatedPoint, leftTrendLineEq, domain);
    const rightTrendLineEq = this.getTrendLineEquation(autofocus.Fittings.RightTrend);
    const rightTrendLine = this.generateTrendLinePoints(false, points, calculatedPoint, rightTrendLineEq, domain);

    return {left: leftTrendLine, right: rightTrendLine};
  }

  getTrendLineEquation(trendFitting) {
    const trendLinePattern = /y\s+=\s+([+-]?[\d.]+)\s+\*\s+x\s+\+\s+([+-]?[\d.]+)/;
    const matches = trendFitting.match(trendLinePattern);
    return {slope: parseFloat(matches[1]), yIntercept: parseFloat(matches[2])};
  }

  generateTrendLinePoints(isLeft, points, calculatedPoint, lineEquation, domain) {
    ...
    const p1y = lineEquation.slope * p1x + lineEquation.yIntercept;
    const p2x = calculatedPoint.Position + extend;
    const p2y = lineEquation.slope * p2x + lineEquation.yIntercept;
